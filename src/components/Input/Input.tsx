...
function Input({data, register, errors}: InputProps): React.ReactElement {
  return (
      <div>
        <label htmlFor={data.name} className="block text-sm font-medium leading-6 text-gray-900">{data.label}</label>
        <div className="relative mt-2 rounded-md shadow-sm">
          <input
              id={data.name}
              aria-invalid={errors[data.name] ? "true" : "false"}
              className={`block w-full rounded-md border-0 py-1.5 pr-10 ring-1 ring-inset focus:ring-2 focus:ring-inset sm:text-sm sm:leading-6 ${errors[data.name] ? 'text-red-900 focus:ring-red-500 placeholder:text-red-300 ring-red-300' : ''}`}
